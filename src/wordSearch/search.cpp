...
void getWord(const Matrix &mtx, Strs &arr, int x1, int y1, int x2, int y2) {
...
void sectionSelect(const Matrix &mtx, Strs &arr, int x1, int y1) {
    for (int d = 1; x1 + d < getCountColumn(mtx); d++) {
        getWord(mtx, arr, x1, y1, x1 + d, y1);
        if (y1 + d < getCountStrs(mtx)) {
            getWord(mtx, arr, x1, y1, x1 + d, y1 + d);
        }
    }
    for (int d = 1; y1 + d < getCountStrs(mtx); d++) {
...
Strs *pointSelect(const Matrix &mtx) {
    ...
    for (int y1 = 0; y1 < getCountStrs(mtx); y1++) {
        for (int x1 = 0; x1 < getCountColumn(mtx); x1++) {
